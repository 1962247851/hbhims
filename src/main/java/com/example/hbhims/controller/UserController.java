...
import com.example.hbhims.service.SysUserRoleRelationService;
import com.example.hbhims.service.SysUserService;
...
public class UserController {
    ...
    @ApiOperation(value = "查询所有专业人员，返回id，姓名，建议评价的平均分，建议的次数")
    @ApiResponses({
            @ApiResponse(code = 200, message = "成功")
    })
    @GetMapping("queryProfessional")
    public JsonResult<?> queryProfessional() {
        List<CustomSysUserProfessional> customSysUserProfessionals = userService.queryAllProfessional();
        return ResultTool.success(customSysUserProfessionals);
    }
